...
var apiKey = "GQCYqg.tofAsQ:ud8VrfHFZkShHZjU";
...
function renderMembers() {
  ...
  online.innerHTML = "Users online (" + (membersList.length === 0
    ? 0
  ...
  for (var index = 0; index < membersList.length; index++) {
    ...
    if (element.clientId !== clientId) {
      html += "<li><small>" + element.clientId +
        ' <button class="btn btn-xs btn-success" onclick=call("' +
        element.clientId + '")>call now</button> </small></li>';
    }
  }
  list.innerHTML = html;
}
